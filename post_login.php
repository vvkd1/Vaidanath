<?php require_once('global.php');
	$un = mysqli_real_escape_string($db->dbh,$_REQUEST['userid']);	
	$pass = mysqli_real_escape_string($db->dbh,md5($_REQUEST['password']));	
	...
	if(isset($_REQUEST['userid']) && !empty($_REQUEST['userid']) && isset($_REQUEST['password']) && !empty($_REQUEST['password']) ) {	
		...
		$sql="SELECT * , DATE_ADD(create_date, INTERVAL ".$rowsetting->password_expiry." DAY) as total_date, CURDATE() as 'current_date' FROM tb_printadmin WHERE userid like binary '".$un."' and password like binary '".$pass."' and password_status = 1 ";

		$row_user = $db->get_row($sql);	
		//echo $sql;
		//print_r($row_user);	
		//die();		
		$count = count((array)$row_user);		
		if($count > 0) {
