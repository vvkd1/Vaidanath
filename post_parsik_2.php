...
error_reporting(E_ALL ^ E_NOTICE);
	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		while ($line = fgets($fh)) 
		{
			...
			$addresses1[$index] = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', $explode[14])); 
			$addresses2[$index] = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', $explode[15])); 
			$addresses3[$index] = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', $explode[16]));
			$addresses4[$index] = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', $explode[17]));
			$addresses5[$index] = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', $explode[18]));
		...
		if(!$entries_found)
		{
			...
			foreach ($data as $key => $explode) {
				...
				'$explode[0]','$cps_micr[$key]','$c_branche_code[$key]','$explode[7]', '$explode[6]',
				'$name',
				'$explode[9]','$explode[10]','$addresses1[$key]','$addresses2[$key]','$addresses3[$key]',
				'$addresses4[$key]','$addresses5[$key]','$explode[19]','$explode[20]','$explode[24]', '$explode[25]',
				'Y', '$explode[29]','$explode[30]',

				'$cps_date[$key]','$cps_date[$key]', '$explode[5]', '$cpsprocess_user_id', '$explode[36]', 
